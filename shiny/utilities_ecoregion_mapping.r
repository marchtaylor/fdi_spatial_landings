...
map_panel_server <- function(input, output, session) {
  ...
  # find index
  observeEvent(input$map_ecoregion_shape_click, {
    
    ## calculate index of ecoregion selected in shape_eco
    idx_1 <- match(input$map_ecoregion_shape_click$id, shape_eco$Ecoregion)
    
    if (input$map_ecoregion_shape_click$group == "Eco_regions") {
      selected_1$groups <- c(selected_1$groups, input$map_ecoregion_shape_click$id)
      
      proxy_1 %>%
        showGroup(group = input$map_ecoregion_shape_click$id) 
      ...
      selected_1$groups <- setdiff(selected_1$groups, input$map_ecoregion_shape_click$group)
      proxy_1 %>%
        hideGroup(group = input$map_ecoregion_shape_click$group) #%>%
